...
// ─── AIS Choice: LANG 2062 + LANG 2061 (both required) ───────────────────────
export function Legacy2023AISChoiceCourses() {
  const navigate = useNavigate();
  const location = useLocation();
  const previousPage = location.state?.previousPage || "/english/2023/ais";
  const affiliation = location.state?.affiliation || "SBM";
  const from = location.state?.from;
  const isFromIsdNo = from === "isd-no";
  const initialDirection = location.state?.direction === "down" ? "-100%" : "100%";
  const [exitDirection, setExitDirection] = useState("up");
  ...
  return (
    <motion.div className="min-h-screen bg-white text-black" initial={{ y: initialDirection }} animate={{ y: 0 }} exit={{ y: exitDirection === "up" ? "-100%" : "100%" }} transition={{ duration: 0.6, ease: "easeInOut" }}>
      <NavBar
        onBack={() => { setExitDirection("down"); setTimeout(() => navigate(previousPage, { state: { direction: "down", affiliation, from, previousPage } }), 300); }}
        onHome={() => { setExitDirection("down"); setTimeout(() => navigate("/", { state: { direction: "down" } }), 300); }}
      />
      <main className="max-w-6xl mx-auto px-6 pt-4">
        <section className="text-center py-10">
          <h2 className="text-5xl md:text-7xl font-semibold tracking-tight mb-6">Required courses</h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
        ...
        <section className="text-center mt-10 pb-16">
          <p className="text-lg text-gray-600 mb-6">After completing these courses, you may also take Advanced Communication courses. Click the button to see what courses are next.</p>
          <button onClick={() => { setExitDirection("up"); setTimeout(() => navigate(isFromIsdNo ? "/english/2023/ais/choice/advanced-communication" : affiliation === "SHSS" ? "/english/2023/ais/shss/advanced-communication" : affiliation === "SSCI" ? "/english/2023/ais/ssci/advanced-communication" : "/english/2023/ais/sbm/advanced-communication", { state: { direction: "up", previousPage: location.pathname } }), 300); }}
            className="inline-flex items-center justify-center min-w-[220px] px-10 py-5 rounded-[32px] bg-gradient-to-b from-blue-50 to-indigo-100 border border-blue-200 text-blue-700 text-lg font-semibold shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
            Continue
          </button>
        </section>
...
export function Legacy2023AISISDCourses() {
  ...
  const isdCourses = [
    ...
    { code: "LANG 4033", title: "Technical Communication II for Civil and Environmental Engineering", url: "https://cle.hkust.edu.hk/courses/lang4033" },
  ...
  return (
    <motion.div className="min-h-screen bg-white text-black" initial={{ y: initialDirection }} animate={{ y: 0 }} exit={{ y: exitDirection === "up" ? "-100%" : "100%" }} transition={{ duration: 0.6, ease: "easeInOut" }}>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-4">
        <section className="text-center py-10">
          <h2 className="text-5xl md:text-7xl font-semibold tracking-tight mb-6">Required course</h2>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
        ...
        <section className="text-center mt-10 pb-16">
          <p className="text-lg text-gray-600 mb-6">After completing this course, you may also take Advanced Communication courses. Click the button to see what courses are next.</p>
          <button onClick={() => { setExitDirection("up"); setTimeout(() => navigate("/english/2023/ais/isd/advanced-communication", { state: { direction: "up", previousPage: location.pathname } }), 300); }}
            className="inline-flex items-center justify-center min-w-[220px] px-10 py-5 rounded-[32px] bg-gradient-to-b from-blue-50 to-indigo-100 border border-blue-200 text-blue-700 text-lg font-semibold shadow-md hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
            Continue
          </button>
        </section>
